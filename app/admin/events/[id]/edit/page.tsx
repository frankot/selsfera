  ...
  if (!event) {
    return (
      <AdminPageWrapper title="Edytuj wydarzenie">
        <div className="text-sm text-foreground1/70">Nie znaleziono wydarzenia.</div>
      </AdminPageWrapper>
    )
  }
  const formEvent = {
    ...
    heroImages: (event.heroImages as unknown as { src: string; alt: string; publicId?: string }[]) ?? [],
    infoBlocks: (event.infoBlocks as unknown as { title: string; type: 'list' | 'paragraphs' | 'raw'; items?: string[]; content?: string }[]) ?? [],
